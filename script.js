...
function drawPebbles() {
  pebbles.forEach((pebble) => {
    ctx.drawImage(pebbleImage, pebble.x - 20, pebble.y - 20, 40, 40); // Adjust width and height here
  });
}
  ...
  ghosts.forEach((ghost) => {
    ...
    if (ghost.dx !== 0) {
      if (ghost.x > ghost.startX + ghost.range || ghost.x < ghost.startX - ghost.range) {
        ghost.dx *= -1;
        ghost.x += ghost.dx; // Prevent getting stuck at the edge
      }
    }
    if (ghost.dy !== 0) {
      if (ghost.y > ghost.startY + ghost.range || ghost.y < ghost.startY - ghost.range) {
        ghost.dy *= -1;
        ghost.y += ghost.dy; // Prevent getting stuck at the edge
      }
    }
  });
}
