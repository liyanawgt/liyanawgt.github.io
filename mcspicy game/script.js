...
function update() {
  ...
  // Check for ghost collisions (now collectible)
  ghosts.forEach((ghost, index) => {
    if (
      ...
      ghosts.splice(index, 1); // Remove ghost on collision
    }
  });

  // Game over if no ghosts or pellets left
...
function draw() {
  ...
  // Draw ghosts
  const ghostSize = Math.min(canvas.width, canvas.height) * 0.1; // Increased size for better visibility
  ghosts.forEach((ghost) => {
