...
function drawPebbles() {
  const pebbleSize = Math.min(canvas.width, canvas.height) * 0.06; // Set size relative to canvas
  pebbles.forEach((pebble) => {
    ctx.drawImage(pebbleImage, pebble.x - pebbleSize / 2, pebble.y - pebbleSize / 2, pebbleSize, pebbleSize);
  });
}
...
function updateGhosts() {
  ghosts.forEach((ghost) => {
    ...
    // Reset stuck ghosts (if stuck for some reason)
    if (ghost.dx === 0 && ghost.dy === 0) {
      ghost.dx = 2; // Default horizontal speed
      ghost.dy = 2; // Default vertical speed
    }
  });
}
